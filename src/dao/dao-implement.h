...
class DaoImplement : public DaoInterface {
  public:
    ...
    DaoImplement(const std::string &mongodb_ip_and_port) : mongodb_(mongodb_ip_and_port), redis_() {
        _init();
    }
  ...
  public:
    ...
    bool destroyAllData() override { return mongodb_.deleteDatabase(DATABASE_NAME); }
